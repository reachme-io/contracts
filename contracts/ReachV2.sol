...
contract ReachV2 is Pausable, ReentrancyGuard {
    ...
    event ProofSignerUpdated(address indexed oldProofSigner, address indexed newProofSigner);
    ...
    constructor(address _authority, address _proofSigner, address _feesReceiver) {
    ...
    function deposit(
        ...
        if (!_verifySignature(_identifier, _amount, _kol, _requester, _responseTime, _refundPercentage, _deadline, _signature)) revert InvalidSignature();

        _deposit(_identifier, _amount, _kol, _requester, _responseTime, _refundPercentage);
    }
    ...
    function _deposit(
        ...
        if (msg.value < MIN_PAYMENT_THRESHOLD || msg.value < _amount) revert InsufficientPayment();
        if (_responseTime > MAX_RESPONSE_TIME) revert InvalidResponseTime();
        if (_kol == address(0) || _requester == address(0)) revert ZeroAddress();
        if (_kol == _requester) revert CannotPaySelf();
        if (_refundPercentage > MAXIMUM_PERCENTAGE || _refundPercentage < MINIMUM_PERCENTAGE) revert InvalidPercentage();
        ...
        uint256 instantAmount = (totalAmount * instantPercentage) / PERCENTAGE_BASE;
        ...
        emit PaymentDeposited(depositId, _identifier, _requester, _kol, totalAmount, kolInstantAmount, escrowAmount);
    }
    ...
    function release(uint256 _depositId) external onlyRole(ENGINE_ROLE) whenNotPaused nonReentrant {
    ...
    function refund(uint256 _depositId) external whenNotPaused nonReentrant onlyRole(ENGINE_ROLE) {
        Deposit storage _deposit = deposits[_depositId];
    ...
    function forceRefund(uint256 _depositId) external nonReentrant {
        Deposit storage _deposit = deposits[_depositId];
        if (_deposit.released || _deposit.refunded) revert AlreadyProcessed();
        if (block.timestamp < (_deposit.timestamp + _deposit.responseTime + 14400)) revert TimeWindowNotElapsed();
        if (_deposit.requester != msg.sender) revert Unauthorized();

        _deposit.refunded = true;

        (bool success, ) = _deposit.requester.call{value: _deposit.escrowAmount}("");
    ...
    function updateProofSigner(address _newProofSigner) external onlyRole(ADMIN_ROLE) {
    ...
    function updatePlatformFee(uint256 _newFee) external onlyRole(ADMIN_ROLE) {
        if (_newFee < MIN_FEE_PERCENTAGE || _newFee > MAX_FEE_PERCENTAGE) revert InvalidFeeRange();
    ...
    function getUserDeposits(address _user, uint256 offset, uint256 limit) external view returns (uint256[] memory) {
        uint256[] storage userDeps = userDeposits[_user];
        if (offset >= userDeps.length) return new uint256[](0);
        uint256 length = limit > userDeps.length - offset ? userDeps.length - offset : limit;
